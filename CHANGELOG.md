...
#### 不要メニューの削除
- サイドバーから「生徒管理」「日程・コース管理」「レポート」の3メニューを削除（未実装・運用外）
...
#### 生徒詳細の「やりとり履歴」を統合表示に拡張
- 従来「お問い合わせ履歴」は生徒からの質問と回答（`inquiries`）のみ表示していたが、**その生徒宛にこちらから送った個別お知らせ（`notices`）も時系列で統合表示**するよう拡張
...
- **事象**：受講中の生徒が実習先変更届を再提出すると、入学時専用のID発行処理が再実行され、既存の `edulioId`・パスワード・`activeAt`（受講開始日）が上書きされていた（土井様の hca002 が hca007 に上書きされる事故が発生。データは復元済み）
...
#### 実習先受け入れ依頼レターを2点新規作成
- **ojt_letter.html**（スクール発行）：実習担当・荘川真理名義。OJT3項目（実技指導・事例検討・修了評価テスト）の説明、詳細資料の後日郵送案内、担当者本人が確認できるメールアドレス提供のお願い（個人情報保護の理由を明記）。生徒氏名・日付は自動差し込み
...
- admin.html の実習先届出一覧・生徒詳細モーダルにも法人名・メールを表示
...
- 施設が「未定」の生徒には進捗欄に「🏥 施設未定」を表示し、不要な「再届出を許可」ボタンを非表示に（誤送信防止）
...
- 未定フロー実装：未定で届出後、施設が決まり次第「実習先変更届」から提出。管理者は「再届出を許可」で生徒に変更届を再表示（自動お知らせ＋メール送信）
